...
# # Правильный запрос для вашего сервиса
# PROMETHEUS_QUERY = f'''
# # Средняя загрузка CPU по всем репликам вашего сервиса
# avg(
#   rate(container_cpu_usage_seconds_total{{
...
def print_configuration():
    ...
    print("╔═══════════════════════════════════════════════════════════════╗")
    print("║           Docker Swarm Autoscaler Configuration               ║")
    print("╠═══════════════════════════════════════════════════════════════╣")
    print(f"║ Service Name:         {SERVICE_NAME:<40}║")
    print(f"║ Prometheus URL:       {PROMETHEUS_URL:<40}║")
    print(f"║ CPU Upper Limit:      {CPU_PERCENTAGE_UPPER_LIMIT}%{'':<36} ║")
    print(f"║ CPU Lower Limit:      {CPU_PERCENTAGE_LOWER_LIMIT}%{'':<36} ║")
    print(f"║ Check Interval:       {CHECK_INTERVAL} sec{'':<33} ║")
    print(f"║ Loop Mode:            {LOOP:<40}║")
    print("╠═══════════════════════════════════════════════════════════════╣")
    
    # Дополнительные переменные окружения
    docker_host = os.getenv('DOCKER_HOST', 'default (unix://var/run/docker.sock)')
    print(f"║ Docker Host:          {docker_host:<40}║")
    
    # Проверяем доступность Docker socket
    docker_socket = '/var/run/docker.sock'
    if os.path.exists(docker_socket):
        print(f"║ Docker Socket:        {'Available':<40}║")
    else:
        print(f"║ Docker Socket:        {'NOT FOUND!':<40}║")
